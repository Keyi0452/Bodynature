...
> = {
  ...
  气虚质: {
    daily: ["早睡早起，午后可小憩；避免久坐与透支。", "注意保暖，减少吹空调直吹。"],
    diet: ["温热饮食，少生冷；可用大枣、山药、莲子煲粥。"],
    sport: ["缓和运动为主：太极/八段锦/散步；循序渐进。"],
    mood: ["避免过度担忧；番茄钟工作法，劳逸结合。"],
  },
  阳虚质: {
    daily: ["重视保暖；腰腹、足部注意御寒。", "沐日光、温水泡脚。"],
    diet: ["温补为主：生姜、羊肉、桂圆；少冷饮生食。"],
    sport: ["中低强度有氧+核心热身；出汗微微即可。"],
    mood: ["减少久坐与阴冷环境；保持积极社交。"],
  },
  阴虚质: {
    daily: ["避免熬夜与过度出汗；房间湿度适中。", "午后适度休息，护眼。"],
    diet: ["滋阴润燥：百合、银耳、沙参、麦冬；少辛辣烧烤。"],
    sport: ["舒缓为主：瑜伽、太极、游泳（不过度）。"],
    mood: ["情绪平稳，学会放慢节奏与深呼吸。"],
  },
  痰湿质: {
    daily: ["清淡饮食，控制甜食与夜宵；规律作息。", "多通风、多走动。"],
    diet: ["祛湿化痰：陈皮、薏米、赤小豆；少油腻奶茶酒。"],
    sport: ["中低强度耐力运动：快走/慢跑/骑行，每周4–5次。"],
    mood: ["减少久坐与沉溺；建立每日步数目标。"],
  },
  湿热质: {
    daily: ["避免熬夜与过度辛辣；注意清洁与通风。"],
    diet: ["清热利湿：绿豆、冬瓜、苦瓜、荷叶；少烤串油炸酒。"],
    sport: ["适度运动+充分补水，避免高温曝晒后大汗久坐。"],
    mood: ["保持心平气和；规律作息减轻内热。"],
  },
  血瘀质: {
    daily: ["避免久坐久立，注意保暖驱寒。", "可热敷/泡脚促进气血。"],
  ...
  气郁质: {
    ...
    sport: ["节律性运动：舞蹈、慢跑、呼吸练习。"],
    mood: ["倾诉/写作/音乐舒压；练习冥想与腹式呼吸。"],
  },
  特禀质: {
    daily: ["尽量减少过敏原暴露；保持室内洁净通风。"],
    diet: ["个别食物过敏应回避；清润饮食。"],
    sport: ["适度、规律；避免花粉季高强度户外。"],
    mood: ["保持情绪稳定，避免过度精神紧张。"],
  },
};
...
export default function App() {
  ...
  const main = result!.mainTypes; // 两个最有倾向的体质（已排除平和质）
    ...
    <div className="min-h-screen" style={{ backgroundImage: THEME.bgGradient, color: THEME.text }}>
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mb-6 rounded-2xl p-4 shadow-sm"
             style={{ background: THEME.cardBg, border: `1px solid ${THEME.border}` }}>
          <div className="text-lg font-semibold mb-1">主要体质（前2名）</div>
          ...
          <div className="grid grid-cols-2 gap-16">
            ...
            <div>
              ...
              <ul className="text-sm space-y-1 mb-6">
                {(Object.keys(trans) as BodyType[]).map((k) => (
                  <li key={k}>{k}：{trans[k].toFixed(1)}</li>
                ))}
              </ul>
              <div className="font-medium mb-2">建议摘要</div>
              <ul className="text-sm list-disc list-inside space-y-1">
                {main.flatMap((t) => ADVICE[t].daily.slice(0,1).concat(ADVICE[t].diet.slice(0,1))).map((s,i)=><li key={i}>{s}</li>)}
              </ul>
            </div>

            {/* 右侧：二维码 */}
            <div className="text-center">
              <img id="poster-qr" alt="qrcode" className="inline-block w-40 h-40" />
              <div className="mt-3 text-sm">{new URL(SITE_URL).host}</div>
            </div>
          </div>
